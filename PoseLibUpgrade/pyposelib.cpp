...
std::pair<CameraPose, py::dict> estimate_relative_pose_3ptupright_wrapper(const std::vector<Eigen::Vector2d> points2D_1,
                                                               const std::vector<Eigen::Vector2d> points2D_2,
                                                               const py::dict &camera1_dict,
                                                               const py::dict &camera2_dict,
                                                               const py::dict &ransac_opt_dict,
    ...
    RansacStats stats =
        estimate_relative_pose_3ptupright(points2D_1, points2D_2, camera1, camera2, ransac_opt, bundle_opt, &pose, &inlier_mask);
...
PYBIND11_MODULE(poselib, m) {
    ...
    m.def("estimate_relative_pose_3ptupright", &poselib::estimate_relative_pose_3ptupright_wrapper, py::arg("points2D_1"),
          py::arg("points2D_2"), py::arg("camera1_dict"), py::arg("camera2_dict"), py::arg("ransac_opt") = py::dict(),
          py::arg("bundle_opt") = py::dict(), "Relative pose estimation with non-linear refinement.");
