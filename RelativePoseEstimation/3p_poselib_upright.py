...
def get_relative_pose(path_img1, path_img2, path_pose1, path_pose2, path_K):
    ...
    #get gravity vector for picture 
    rgc3 = np.identity(3)
    with open(path_pose1, 'r') as f:
        rot_row = f.readlines()
        #print(rot_row)
        for i in range(3):
            r1, r2, r3, r4 = rot_row[i].split()
            rgc3[i, :] = np.array((np.float32(r1), np.float32(r2), np.float32(r3)))
    g1 = rgc3[2, :, None]
    ...
    #get gravity vector for picture 2
    rgc32 = np.identity(3)
    with open(path_pose2, 'r') as f:
        rot_row = f.readlines()
        for i in range(3):
            r1, r2, r3, r4 = rot_row[i].split()
            rgc32[i, :] = np.array((np.float32(r1), np.float32(r2), np.float32(r3)))
    g2 = rgc32[2, :, None]
